...
public class Maze {
	...
	public void printWithSolution(IASolution solution) {
		for (int i = 0; i < this.mazeArray.length; i++) {
			for (int j = 0; j < this.mazeArray.length; j++) {
				if (checkIfCoordIsPartOfSolutionPath(i, j, solution)) {
					System.out.print('*');
	...
	private boolean checkIfCoordIsPartOfSolutionPath(int i, int j, IASolution solution) {
	...
	public String toString() {
		...
		String ret = "";
		ret += "Laberinto - Dimensión(" + getDimension() + 
				") - Factor(" + this.fillFactor + ") - Paredes("
				+ getNumberOfWalls() + ")\n";
